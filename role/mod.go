...
var (
	...
	rolesPath = filepath.Join(".", "db", "roles.json")
)
...
func save() error {
	...
	return os.WriteFile(rolesPath, data, 0o600)
}
...
func Create(name, description string, permissions []Permission) (Role, error) {
	if name == "" {
...
func Update(name string, description string, permissions []Permission) (Role, error) {
	if name == "" {
		return Role{}, errors.New("role name is required")
	}
	load()
